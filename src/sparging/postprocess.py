...
def summarize_decay(
    ...
    tau_pred_SPP = results.sim_input.get_tau_SPP()
    residual = get_residual_fraction(inv, times, t_0, t_end)
    rel_error = ((tau_fit - tau_pred) / tau_pred).to("dimensionless")
    SPP_rel_error = ((tau_fit - tau_pred_SPP) / tau_pred_SPP).to("dimensionless")
