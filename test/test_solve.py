...
@pytest.fixture
def standard_input():
    return get_sim_input_standard()


@pytest.fixture
def standard_simulation():
    my_input = get_sim_input_standard()  # can't use standard_input fixture
...
def test_model_solve_missing_input(standard_simulation):
    ...
    """
    # BUILD
    del standard_simulation.sim_input.temperature  # missing required parameter

    # TEST
    with pytest.raises(
        AttributeError, match="'SimulationInput' object has no attribute 'temperature'"
    ):
...
def test_model_solve_wrong_input(standard_simulation):
    ...
    """
    # BUILD
    standard_simulation.sim_input.temperature = 3 * ureg("m^2/s")
...
def test_source_T_normalization(case, standard_input):
    ...
    n_result = ureg.Quantity(output.n_T2_salt_series[-1], "molT2").magnitude
